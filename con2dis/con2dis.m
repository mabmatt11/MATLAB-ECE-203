...
%globals
Ver = 'CON2DIS ver. 2.27';  % Version string for figure title
...
switch action
    ...
    case {'Editbox2','Slider2'}
        ...
        if h.wflag == 1   
            ...
            if h.MATLABVER>=8.4
                set(h.TextFs,'String',['$$\bf\hat\omega_s = ' sprintf('%.1f',pi2*FsN) '$$ (rad/s)'],'Interpreter','Latex');
            else
                set(h.TextFs,'String',sprintf('\\omega_s = %.1f (rad/s)',pi2*FsN),'Interpreter','tex');
            end
        else   
            Fo = sprintf('%.1f',FoN);
            set(h.Editbox1,'String',Fo);
            ...
            if h.MATLABVER>=8.4
                set(h.TextFs,'String',['$$\bf f_s = ' sprintf('%.1f',FsN) '$$ (Hz)'],'Interpreter','Latex');
            else
                set(h.TextFs,'String',sprintf('f_s = %.1f (Hz)',FsN),'Interpreter','tex');
            end
        end
        ...
        if length(time2) > 12
            time2a = 0:2/FsN:1;
            set(h.Axis2,'XTick',time2a,'XTickLabel',2*(0:length(time2a)-1)');
        else
            set(h.Axis2,'XTick',time2,'XTickLabel',(0:length(time2)-1)');
        end
    ...
    case 'ShowW' 
        ...
        if strcmp(show,'off')
            ...
            if h.MATLABVER>=8.4
                xlabel('$$\bf\hat\omega$$ (rad/s)','Interpreter','Latex')
            else
            ...
            if h.MATLABVER>=8.4
                set(h.TextFo,'String',['$$\bf\hat\omega_o = ' sprintf('%.1f',WoN) '$$ (rad/s)'],'Interpreter','Latex');
                set(h.TextFs,'String',['$$\bf\hat\omega_s = ' sprintf('%.1f',WsN) '$$ (rad/s)'],'Interpreter','Latex');
            else
                set(h.TextFo,'String',sprintf('\\omega_o = %.1f (rad/s)',WoN),'Interpreter','tex');
                set(h.TextFs,'String',sprintf('\\omega_s = %.1f (rad/s)',WsN),'Interpreter','tex');
            end
            ...
            if h.MATLABVER>=8.4
                set(h.TextFo,'String',['$$\bf f_o = ' sprintf('%.1f',FoN) '$$'],'Interpreter','Latex');
                set(h.TextFs,'String',['$$\bf f_s = ' sprintf('%.1f',FsN) '$$'],'Interpreter','Latex');
            else
                set(h.TextFo,'String',sprintf('f_o = %.1f (Hz)',FoN),'Interpreter','tex');
                set(h.TextFs,'String',sprintf('f_s = %.1f (Hz)',FsN),'Interpreter','tex');
            end
    ...
    case 'ShowW_hat' 
        ...
        if strcmp(show_hat,'off')
            ...
            set(h.Axis3,'XTick',0.5*(-2:2),'XTickLabel',{'-2p';'-p';'0';'p';'2p'}...
            ...
            % change text labels
            set(gcbf,'CurrentAxes',h.Axis3);
            set(h.Axis3,'XTickLabel',{'-1';'-0.5';'0';'0.5';'1'},...
                'FontWeight','bold','FontSize',h.FontSize);  
            
            %xlabel('{}_{^{\fontsize{11} f = f_o / f_s}}^{\^}')
            if h.MATLABVER>=8.4
                xlabel('$$\bf f=\frac{f_o}{f_s}$$','Interpreter','Latex')
            else
                set(h.Axis3,'FontName','Helvetica');
                xlabel('f = f_o / f_s','Interpreter','tex')
            end
